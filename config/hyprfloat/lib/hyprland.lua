...
function hyprland.calculate_effective_area(monitor, border_gap)
    ...
    -- Swap width/height for rotated monitors (transform 1 or 3 = 90/270 degrees)
    ...
    -- reserved array from Hyprland JSON: [left, top, right, bottom] (1-indexed in Lua)
    local res_left   = monitor.reserved[1]
    local res_top    = monitor.reserved[2]
    local res_right  = monitor.reserved[3]
    local res_bottom = monitor.reserved[4]

    local screen_x = monitor.x + res_left
    local screen_y = monitor.y + res_top
    local screen_w = lw - res_left - res_right
    local screen_h = lh - res_top - res_bottom
