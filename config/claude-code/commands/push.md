...
allowed-tools: Bash(git status:*), Bash(git diff:*), Bash(git add:*), Bash(git commit:*), Bash(git push:*), Bash(git log:*), Bash(git remote:*)
---
...
4. Stage only the relevant files by name -- do NOT use `git add -A` or `git add .`
5. Generate a concise conventional commit message based on the actual diff
6. Commit with that message
7. Push to the remote on the current branch (usually main). If multiple remotes exist, push to the one owned by `clearcmos`.
